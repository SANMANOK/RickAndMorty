...
describe("Tests de Rutas", () => {
  ...
  describe("GET /rickandmorty/login", () => {
    it("Informacion sea correcta y nos de acceso", async () => {
      const {body} = await agent.get(
        "/rickandmorty/login?email=sanman@gmail.com&password=sanmanok"
      );
    ...
    it("Informacion sea incorrecta y nos bloquea", async () => {
      const {body} = await agent.get(
        "/rickandmorty/login?email=sanman@gmail.com&password=estamal"
      );
  ...
  describe("POST /rickandmorty/fav", () => {
    const char1 = {id: 1, name: "Sanman"};
    const char2 = {id: 2, name: "Justi"};
